...
应用3：假设线性表A=(a1,a2,...,am)，线性表B=(b1,b2,...,bn)，
    现要求设计一个算法，使用带头结点的单链表存储结构，按照下列规则合并A、B为线性表C。
    即：A和B的元素间隔排列，且使用A和B的原存储空间，且B不再单独存在。
    输入中的单链表的长度不得在该算法中利用，仅作为建表使用。
...
template <class Iter>
void Merge_L(LinkList<Iter> &A, LinkList<Iter> &B)
{
    LinkList<Iter> p = A->getNext();
    LinkList<Iter> q = B->getNext();
    LinkList<Iter> r = NULL;
    while (p && q)
    {
