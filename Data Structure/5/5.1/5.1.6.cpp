/*
在二叉树的二叉链表存储形式建立的基础上，使用递归的程序设计方法，设计并完成计算二叉树中以元素值x的结点为根结点的（子）树的高度的算法。完成后将其加入到二叉树的ADT基本操作集中。
初始条件：二叉树T存在，x是T中某个结点。
操作结果：若x是T的结点，则返回以它为根结点的（子）树的高度（查找成功）。否则返回0（查找失败）。
*/
