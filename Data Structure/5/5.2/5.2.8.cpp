/*
在二叉树的二叉链表存储形式建立的基础上，设计并完成按树状打印二叉树的算法。
*/
...
public:
    ...
    void inOrderWithStack() // 中序遍历非递归算法
    {
        std::stack<BiTNode<Iter> *> S;
        auto p = root;
        while (p || !S.empty())
    ...
    bool drawTree() // 画二叉树
    {
        ...
        if (this->isEmpty())
            return false;
        drawTree_Cursive(root, layer);
        return true;
    }
    void drawTree_Cursive(BiTNode<Iter> *node, int i)
    {
        if (!node)
            return;
        if (node->rchild)
            drawTree_Cursive(node->rchild, i + 1);
        for (int j = 1; j <= i; j++)
            std::cout << "  ";
        std::cout << node->data << " " << std::endl;
        if (node->lchild)
            drawTree_Cursive(node->lchild, i + 1);
    }
