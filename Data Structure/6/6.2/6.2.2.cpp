/*
 * 设计并实现一个算法，在已存在的图中返回G中指定顶点u的第一个邻接顶点的位序（顶点集）。
 * 若顶点在G中没有邻接顶点，则返回-1。
 */
...
public:
    ...
    int getFirstAdjVertexIndex(int vertex) const
    {
        EdgeNode *node = this->vertex[vertex].firstEdge;
        if (node)
...
int main()
{
    ...
    cout << endl;
    graph.printGraph();
    cout << endl << endl << graph.getFirstAdjVertexIndex(v);
    return 0;
}
